...
const PomodoroTimer: React.FC<PomodoroTimerProps> = ({
  ...
  function handleModeComplete(completedMode: PomodoroMode) {
    // Show notification
  ...
  // Update start/end time display when timer is running
  useEffect(() => {
    if (isRunning && time > 0) {
      const now = new Date();
      ...
      // Calculate start time from end time - total duration
      // This gives us the actual start time of the session
      const calculatedStartTime = new Date(
        calculatedEndTime.getTime() - modeDuration * 1000
      );
      setStartTime(calculatedStartTime);
    } else if (!isRunning && time === 0) {
      // Reset times when timer completes
      setStartTime(null);
      setEndTime(null);
    }
  }, [isRunning, time, mode]);
